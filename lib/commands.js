...
function checkRepoBinding(repo, user, users) {
  ...
  const sshMatch = keyAbs && ssh.includes(keyAbs.replace(/\\/g, '/'));
...
function checkIdentityDrift({ force = false, yes = false } = {}) {
  ...
  const keyAbs = expandHome(u.key || '');
  const keyOk = keyAbs && ssh.includes(keyAbs.replace(/\\/g, '/'));
...
/** 取身份并校验；needKeyFile 时额外要求密钥文件存在（只有 SSH 场景才需要） */
...
function gitProbe(args, { cwd = process.cwd(), env } = {}) {
  ...
  try {
    ...
    return { ok: false, stderr: String((e && e.stderr) || (e && e.message) || '') };
  }
}

function requireRepoForNet(cmd) {
  if (!isInsideRepo()) {
    throw new Error(`当前目录不是 git 仓库，无法 ${cmd}。用 \`ego clone <身份> <地址>\` 克隆，或先 \`git init\`。`);
  }
}
...
export function cmdLsRemote(user, url) {
  ...
  const r = gitProbe(['ls-remote', target], ssh ? { env: borrowEnv(keyAbs) } : {});
  if (r.ok) {
...
export async function cmdImport(file, { yes = false } = {}) {
  ...
  if (data.keys && typeof data.keys === 'object') {
    for (const [name, k] of Object.entries(data.keys)) {
      if (k && k.priv) {
        writeFileSync(join(homedir(), '.ssh', name), Buffer.from(k.priv, 'base64'));
        if (k.pub) writeFileSync(join(homedir(), '.ssh', name + '.pub'), Buffer.from(k.pub, 'base64'));
      }
    }
    ...
    saveUsers(users);
    echo(`已恢复 ${Object.keys(data.keys).length} 个 SSH 密钥到 ~/.ssh/`);
  }
