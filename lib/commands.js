...
export function cmdAdd(user, opts) {
  ...
  const users = loadUsers();
  const prev = users[user] || {};
  // 必须展开旧值：否则重跑 add 会静默丢掉 isGlobal 等既有字段
  users[user] = {
    ...prev,
    name: flagStr(opts.name) || prev.name || '',
    email: flagStr(opts.email) || prev.email || '',
    key: flagStr(opts.key) || prev.key || ''
  };
  saveUsers(users);
  echo(`已注册身份 ${user}。可用 \`ego init ${user}\` 在当前仓库绑定。`);
  if (prev.isGlobal) echo(`（保留该身份的全局标记 ★${user}；如需刷新 --global 配置请跑 \`ego sync ${user}\`）`);
}
...
// ---- 身份变更同步 ----
/** 该路径是否是 git 仓库（用于识别注册表里的脏记录） */
function isRepoAt(p) {
  try {
    return !!gitOut(['-C', p, 'rev-parse', '--show-toplevel']);
  } catch {
    return false;
  }
}
...
/**
 * 把注册表里的身份信息（name/email/key）同步到所有绑定该身份的仓库。
 * 典型场景：`ego add <user> --email X` 改了邮箱、或 `ego key-new` 轮换了密钥之后，
 * 各仓库 .git/config 里仍是旧值（ego 不会自动传播，这里补上）。
 * - 只写 user.name/email；core.sshCommand 仅在"该仓库按 SSH 认证"时写（与漂移检查同一判定）
 * - --dry-run 只列计划不落盘；交互模式确认后落盘（--yes 跳过确认；无 TTY 直接执行）
 * - 目标是全局身份时，同时刷新 --global 配置
 */
export async function cmdSync(user, { dryRun = false, yes = false } = {}) {
  const users = loadUsers();
  const repos = loadRepos();
  if (user && !users[user]) throw new Error(`未找到身份 ${user}。用 \`ego users\` 查看已注册身份。`);
  const targets = user ? [user] : Object.keys(users);
  if (!targets.length) {
    echo('没有已注册身份。');
    return;
  }
  const entries = Object.entries(repos).filter(([, u]) => targets.includes(u));
  const globals = targets.filter((t) => users[t] && users[t].isGlobal);
  if (!entries.length && !globals.length) {
    echo(user ? `身份 ${user} 没有绑定任何仓库，也没有全局配置需要更新。` : '没有需要同步的仓库。');
    return;
  }

  const plan = [];
  const skipped = [];
  for (const [repo, owner] of entries) {
    const u = users[owner];
    if (!u) {
      skipped.push(`${repo}（绑定身份 ${owner} 不存在，先 \`ego add ${owner}\`）`);
      continue;
    }
    if (!existsSync(repo)) {
      skipped.push(`${repo}（目录不存在）`);
      continue;
    }
    if (!isRepoAt(repo)) {
      skipped.push(`${repo}（不是 git 仓库）`);
      continue;
    }
    const name = gitConfigAt(repo, 'user.name');
    const email = gitConfigAt(repo, 'user.email');
    const ssh = gitConfigAt(repo, 'core.sshCommand');
    const keyAbs = typeof u.key === 'string' && u.key.trim() ? expandHome(u.key) : '';
    const wantKey = keyRelevantAt(repo);
    const nameOk = name === u.name && email === u.email;
    const keyOk = !wantKey || (!!keyAbs && ssh.includes(keyAbs.replace(/\\/g, '/')));
    if (nameOk && keyOk) continue;
    plan.push({ repo, u, keyAbs, wantKey, name, email, keyOk, keyMissing: wantKey && !keyAbs });
  }

  echo(`▶ 同步身份: ${targets.join(', ')}（绑定仓库 ${entries.length} 个）`);
  for (const p of plan) {
    echo(`  ⚠ ${p.repo}`);
    const from = `${p.name || '(未设)'} <${p.email || '(未设)'}>`;
    const to = `${p.u.name} <${p.u.email}>`;
    if (from !== to) echo(`       ${from}  →  ${to}`);
    if (p.keyMissing) echo('       ⚠ 该身份未配置密钥，跳过 core.sshCommand（SSH 远端会认证失败）');
    else if (p.wantKey && !p.keyOk) echo(`       core.sshCommand → ${sshCommandFor(p.keyAbs)}`);
  }
  if (!plan.length && entries.length) echo('  ✅ 这些仓库的配置都已是最新，无需修改。');
  for (const s of skipped) echo(`  ⏭ ${s}`);
  for (const g of globals) echo(`  ℹ ${g} 是全局身份：同时刷新 --global 配置（${users[g].name} <${users[g].email}>）`);

  const nothingToDo = !plan.length && !globals.length;
  if (dryRun) {
    echo('');
    echo(nothingToDo ? '（--dry-run：没有需要修改的内容）' : '（--dry-run：以上为将要修改的内容，未做任何改动）');
    return;
  }
  if (nothingToDo) return;
  if (!yes && canPrompt()) {
    const a = await prompt('确认写入上述仓库的 .git/config? (Y/n): ');
    if (a.toLowerCase() === 'n') {
      echo('已取消，未做任何修改。');
      return;
    }
  ...
  let changed = 0;
  for (const p of plan) {
    git(['config', 'user.name', p.u.name], { cwd: p.repo });
    git(['config', 'user.email', p.u.email], { cwd: p.repo });
    if (p.wantKey && p.keyAbs) git(['config', 'core.sshCommand', sshCommandFor(p.keyAbs)], { cwd: p.repo });
    changed++;
    echo(`  ✅ ${p.repo}  →  ${p.u.name} <${p.u.email}>`);
  }
  for (const g of globals) {
    ...
    git(['config', '--global', 'user.name', u.name]);
    git(['config', '--global', 'user.email', u.email]);
    if (keyAbs) git(['config', '--global', 'core.sshCommand', sshCommandFor(keyAbs)]);
    echo(`  ✅ --global 配置已刷新为 ${g}（${u.name} <${u.email}>）`);
  }
  echo('');
  echo(`同步完成：更新 ${changed} 个仓库${globals.length ? `，并刷新 ${globals.length} 个全局身份配置` : ''}。`);
  echo('可用 `ego check` 复核一致性。');
}
